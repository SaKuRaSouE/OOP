    ...
    public static void getData(String data[][]) {
        for (int r = 0; r < data.length; r++) {
            for (int c = 0; c < data[r].length; c++) {
                if (c == 0)
                    data[r][c] = JOptionPane.showInputDialog("Enter name " + (r + 1) + " : ");
                else
                    data[r][c] = JOptionPane.showInputDialog("Enter salary " + (r + 1) + " : ");
            }
        }
    }

    public static void displayData(JTextArea text1, JTextArea text2, String d[][]) {
